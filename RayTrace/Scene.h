...
public:
	...
	bool IsSquare(void) { return (type == SceneObjectType::Square);}
};
...
/* single square class */
class SceneSquare : public SceneObject{
public:
	std::string material[4];
	Vector vertex[4];
	Vector normal[4];
	float u[4], v[4];

	//---Constructors---
	SceneSquare(void):SceneObject("Square", SceneObjectType::Square){}
	SceneSquare (std::string nm) : SceneObject (nm, SceneObjectType::Square) {}
};
/*
...
bool Scene::Load (char *filename)
{
	...
	if (!tempNode.isEmpty ())
	{
		...
		for (unsigned int n = 0; n < numObjects; n++)
		{
			...
			else if(!strcasecmp(tempObjectNode.getName (), "square")){
				// Load a Triangle
				XMLNode vertexNode;
				SceneSquare *tempSquare = new SceneSquare ();
				tempSquare->name = CHECK_ATTR(tempObjectNode.getAttribute("name"));
				tempSquare->scale = ParseXYZ (tempObjectNode.getChildNode("scale"));
				tempSquare->rotation = ParseXYZ (tempObjectNode.getChildNode("rotation"));
				tempSquare->position = ParseXYZ (tempObjectNode.getChildNode("position"));
				
				// Load Vertex 0
				vertexNode = tempObjectNode.getChildNodeWithAttribute ("vertex", "index", "0");
				tempSquare->material[0] = CHECK_ATTR(vertexNode.getAttribute("material"));
				tempSquare->vertex[0] = ParseXYZ (vertexNode.getChildNode("position"));
				tempSquare->normal[0] = ParseXYZ (vertexNode.getChildNode("normal"));
				tempSquare->u[0] = atof (CHECK_ATTR(vertexNode.getChildNode("texture").getAttribute("u")));
				tempSquare->v[0] = atof (CHECK_ATTR(vertexNode.getChildNode("texture").getAttribute("v")));

				// Load Vertex 1
				vertexNode = tempObjectNode.getChildNodeWithAttribute ("vertex", "index", "1");
				tempSquare->material[1] = CHECK_ATTR(vertexNode.getAttribute("material"));
				tempSquare->vertex[1] = ParseXYZ (vertexNode.getChildNode("position"));
				tempSquare->normal[1] = ParseXYZ (vertexNode.getChildNode("normal"));
				tempSquare->u[1] = atof (CHECK_ATTR(vertexNode.getChildNode("texture").getAttribute("u")));
				tempSquare->v[1] = atof (CHECK_ATTR(vertexNode.getChildNode("texture").getAttribute("v")));

				// Load Vertex 2
				vertexNode = tempObjectNode.getChildNodeWithAttribute ("vertex", "index", "2");
				tempSquare->material[2] = CHECK_ATTR(vertexNode.getAttribute("material"));
				tempSquare->vertex[2] = ParseXYZ (vertexNode.getChildNode("position"));
				tempSquare->normal[2] = ParseXYZ (vertexNode.getChildNode("normal"));
				tempSquare->u[2] = atof (CHECK_ATTR(vertexNode.getChildNode("texture").getAttribute("u")));
				tempSquare->v[2] = atof (CHECK_ATTR(vertexNode.getChildNode("texture").getAttribute("v")));

				//Load Vertex 3
				vertexNode = tempObjectNode.getChildNodeWithAttribute ("vertex", "index", "3");
				tempSquare->material[3] = CHECK_ATTR(vertexNode.getAttribute("material"));
				tempSquare->vertex[3] = ParseXYZ (vertexNode.getChildNode("position"));
				tempSquare->normal[3] = ParseXYZ (vertexNode.getChildNode("normal"));
				tempSquare->u[3] = atof (CHECK_ATTR(vertexNode.getChildNode("texture").getAttribute("u")));
				tempSquare->v[3] = atof (CHECK_ATTR(vertexNode.getChildNode("texture").getAttribute("v")));

				m_ObjectList.push_back (tempSquare);
			}
