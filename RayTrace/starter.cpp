...
unsigned int g_X_1=0 , g_Y_1 = 0;
bool g_bRayTrace = false;
bool g_bRayTraceSecond = false;
...
void myinit()
{
	...
	g_RayTrace.setRayTrace();
	g_RayTrace_1.setRayTrace();
...
void doIdle()
{
	...
	else if(g_bRayTraceSecond){
		g_ScreenBuffer[g_Y_1][g_X_1] = g_RayTrace_1.CalculatePixel (g_X_1, g_Y_1);

		// Move to the next pixel
		g_X_1++;
		if (g_X_1 >= WINDOW_WIDTH)
		{
			// Move to the next row
			g_X_1 = 0;
			g_Y_1++;
...
int main (int argc, char ** argv)
{
	...
	if(!g_RayTrace_1.m_Scene.Load("snowman1.xml")){
		printf ("failed to load scene\n");
		exit(1);
	}
	glutInit(&argc,argv);
	...
	glutAddMenuEntry("Render RayTrace with 2 lights", 3);
